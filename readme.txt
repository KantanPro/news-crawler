...
**新機能（v2.3.53）:**
* 2025年9月9日リリース
* セキュリティ強化と評価値管理機能の大幅改善
  * OpenAI APIキーの暗号化機能を実装し、セキュリティマネージャーによる安全な管理を実現
  * 既存の平文APIキーを自動的に暗号化するマイグレーション機能を追加
  * 設定画面でのAPIキーマスク表示を実装し、セキュリティを向上
  * 投稿削除やステータス変更時の評価値キャッシュ無効化機能を追加
  * 投稿作成後の評価値復元機能とバックアップ機能を実装
  * 投稿作成時のステータス指定機能を追加し、柔軟な投稿管理を実現
* セキュリティとデータ管理の信頼性を大幅に向上
