...
**新機能（v2.3.43）:**
* 2025年9月6日リリース
* WordPress標準の更新通知に完全対応：アップデータを `plugins_loaded`（優先度1）で早期初期化
* 更新チェック時の防御的初期化（`$transient === null` を安全に初期化）
* 更新レスポンスに `id` と `plugin` を付与し照合の安定性を向上
* 投稿作成結果のメッセージで投稿IDのエラーハンドリングを改善（エラー時は「エラー」と表示）
* 設定管理まわりの軽微な改善（表示や権限処理の安定化）
