...
**新機能（v3.3.21）:**
* X 未シェア待ち行列に公開後 14 日以内の記事のみを対象とする期限フィルタを追加
  * 古すぎる未シェア記事が X シェア対象から自動的に除外されるように改善

**新機能（v3.3.20）:**
* X 未シェア待ち行列の自動消化条件を改善
  * 新規ブログのシェアが API エラー等で失敗し日次枠に余裕がある場合、待ち行列から 1 件消化する
  * 新規シェアが成功して日次枠を使い切った場合のみ待ち行列を見送る
* X シェアの HTTP 403 失敗時にシェアログへ対処ヒントを表示（OAuth 2.0 / 1.0a）
* 未シェア待ち行列の管理画面説明文を更新
...
**新機能（v3.3.18）:**
* X シェアログに新規ブログ件数を明記
  * 未シェア待ち行列の消化／スキップ時に「新規ブログは N 件」を表示
  * 新規ブログ作成時も待ち行列を消化しない理由をシェアログに記録

**新機能（v3.3.17）:**
* ブログ記事の未投稿待ち行列を追加
  * ソースで複数ヒットしても投稿上限で作れなかった記事を待ち行列に保持
  * 次回以降の自動投稿で古い順に消化（フィードから消えても再取得不要）
  * 自動投稿設定画面に待ち行列の件数・先頭記事を表示
...
* コアサーバー向けに自動投稿 cron の PHP 検出を再修正
  * `php -r` 非対応（CGI）でも動くよう `php -v` でバージョン判定
  * `php81cli` / `php80cli` などコアサーバー公式 CLI パスを優先
  * 必要時は `news-crawler-php.path` で PHP フルパスを指定可能

**新機能（v3.3.14）:**
* 自動投稿 cron が古い CLI PHP で Fatal になる問題を修正
  * `news-crawler-cron.sh` で PHP 8.1 以上のバイナリを優先選択
  * `/usr/local/bin/php`（古い CLI）による WordPress 読み込み失敗を回避
...
* X OAuth 1.0a 設定画面の表記を Developer Portal に合わせて改善
  * API Key / API Secret をコンシューマーキー / コンシューマーシークレットに変更
  * Access Token 系をアクセストークン / アクセストークンシークレットに変更
  * コンシューマーキー・アクセストークンに「（生成する）」を表示
  * Portal の画面構成（OAuth 1.0 キー、アクセストークン）に沿った説明文に更新
  * 接続診断・エラーメッセージも同表記に統一（OAuth 2.0 との混同防止）

**新機能（v3.3.11）:**
* 更新通知は出るが更新画面に News Crawler が表示されない不具合を修正
  * WordPress 更新一覧は package URL 必須のため、公開アーカイブ URL を必ず生成
  * GitHub API レート制限時はキャッシュから更新情報を復元
  * 更新 transient へ自プラグイン情報を明示的に同期
  * 管理画面通知に「今すぐ更新」リンクを追加

**新機能（v3.3.10）:**
* 「未シェア投稿を再試行」でシェア済み投稿が再投稿される不具合を修正
  * force モードでシェア済みメタを削除していた問題を解消
  * `_x_post_id`・シェアログの成功記録も参照してシェア済みを判定
  * 未シェア件数の表示をシェア済み除外後の件数に合わせて改善

**新機能（v3.3.9）:**
* WordPress 管理画面からのプラグイン更新失敗を修正
  * GitHub zipball（API URL）の代わりに公開アーカイブ URL を使用（レート制限・403 回避）
  * KantanProEX 等の他プラグイン更新と干渉しないよう自プラグインのみダウンロード処理
  * キャッシュ済み zipball URL を公開 URL に自動変換

**新機能（v3.3.8）:**
* X OAuth 2.0 の Access Token 期限表示が更新されない問題を修正
  * OAuth トークンを専用オプションのみから読み取るよう変更（basic_settings の古い期限が残る不具合を解消）
  * 「設定を保存」時に basic_settings へ OAuth トークンを戻さないよう変更
  * 診断パネル表示時に期限切れなら Refresh Token で自動更新を試行
  * 専用オプション読み込み時に basic_settings へ残った OAuth キーを自動除去

**新機能（v3.3.7）:**
* X 自動シェアの OAuth 2.0 認証エラー（HTTP 401）を改善
  * 401 時にトークン自動更新の成否を判定し、再接続手順を含む日本語メッセージをシェアログに表示
  * Client Secret 未設定・復号不可・Refresh Token 不足時の対処案内を追加
  * トークン自動更新失敗時に debug.log へ HTTP ステータスとレスポンスを記録
* X 接続診断パネルに Access Token の有効期限を表示（期限切れ時は再接続を案内）
* 「未シェア投稿を再試行」を 1 件ずつ実行に変更（レート制限対策）

**新機能（v3.3.6）:**
* X への自動シェアに1日あたりの上限を設定可能に
  * 自動投稿設定の X 一般設定に「1日の自動シェア上限」を追加（0 で無制限、最大 500 件）
  * 本日の自動シェア数を設定画面に表示
  * 上限到達時は自動シェアをスキップし、シェアログに理由を記録
  * テスト投稿・未シェア投稿の手動再試行は上限の対象外

**新機能（v3.3.5）:**
* アイキャッチ生成プロンプト（基本）の初期値を刷新
  * 基本設定＞機能設定のテキストエリアに、中央テキスト必須・2Dイラスト・背景全面描画などを含む詳細プロンプトを初期値として設定
  * 未設定または旧デフォルト（短い英語プロンプト）のサイトは自動マイグレーションで新初期値を適用
  * 設定リセット時も新初期値を使用
  * 構造化プロンプト（【】形式）は禁止語句の自動除去対象外に変更

**新機能（v3.3.4）:**
* アイキャッチ画像生成を大幅改善
  * 基本設定＞機能設定に「アイキャッチ生成プロンプト（基本）」を追加
  * 投稿編集画面の「キーワード」を「画像生成指示プロンプト」テキストエリアに変更（投稿メタに保存）
  * ユーザー指示を最優先で反映（PRIMARY IMAGE DIRECTION）
  * 常に2Dイラストで生成（実写・モノクロ・写真風を禁止するプロンプト強化）
  * 再生成時は既存アイキャッチを削除し毎回新規生成（過去画像の再利用を廃止）
  * プロンプト指定時は AI 失敗でもテンプレート／Unsplash へフォールバックしない
  * AI 失敗時のテンプレートフォールバックに図形イラストを追加（文字なし）
* OpenAI 画像 API を GPT Image 1.5 へ移行（DALL-E 2/3 廃止対応）
  * `response_format` 非対応エラーを修正（base64 レスポンス対応）
  * デフォルトモデルを `gpt-image-1.5` に変更（旧 DALL-E 設定は自動移行）

**新機能（v3.3.3）:**
* X 自動シェアの安定性を改善
  * HTTP 403 / 429 時に最大 3 回まで指数バックオフで自動再試行（OAuth 1.0a / 2.0）
  * OAuth 2.0 トークンを期限切れ 5 分前から更新（cron 長時間実行時の失敗回避）
  * 「未シェア投稿を再試行」で投稿間に 3 秒の待機を追加（レート制限対策）
  * 「未シェア投稿を再試行」は未シェア投稿のうち最大 3 件ずつ実行（レート制限対策）
  * 403 失敗時にシェアログへ投稿文字数を表示（280 文字超過の切り分け用）

**新機能（v3.3.2）:**
* 管理画面の利用規約表示を左寄せに変更

**新機能（v3.3.1）:**
* WooCommerce 向け商品説明テンプレートを docs に追加（バージョン番号に依存しない文言）

**新機能（v3.3.0）:**
* GPL v2 or later にライセンス表記を統一（readme.txt / license.txt）
* プラグイン内ライセンス認証・ライセンス設定メニューを廃止（WooCommerce ダウンロード販売向け）
* 自動投稿設定を含む全機能をライセンス不要で利用可能に変更

**新機能（v3.2.29）:**
* 404・削除済みページを記事として投稿しないよう改善
  * HTTP 4xx/5xx のURLは取得時点で除外
  * 「ページが見つかりません」「お探しのページは削除されています」等のタイトル・本文を検出してスキップ
  * 個別ページ取得・RSS詳細取得・クロール・要約生成の各段階で二重チェック

**新機能（v3.2.28）:**
* 2026年5月29日リリース
* アイキャッチ画像をよりカラフルに、黒背景を禁止
  * DALL-E プロンプトに明るいハイパーカラフル背景と黒背景禁止を明示
  * シーン・配色・ライティングを明るい高彩度向けに刷新
  * テンプレート生成のグラデーション配色を明るい色に変更

**新機能（v3.2.27）:**
* 2026年5月28日リリース
* SEOタイトル生成を共有記事・動画の内容に沿うよう改善
  * 要約ソースから元タイトルを抽出し、タイトル生成の最優先材料に使用
  * ターゲットキーワードは内容と関連する場合のみ任意で反映（無理な挿入を禁止）
  * 「見積書作成」など記事に無い話題をタイトルに入れないようプロンプトを強化
  * AI要約のキーワード指示も同方針に統一

**新機能（v3.2.26）:**
* 2026年5月28日リリース
* 管理画面のバージョン表示を `news_crawler_get_version()` に統一
  * プラグインヘッダーの Version を動的取得（定数とのズレを防止）
  * 自動投稿設定・OGP画像設定など全管理画面タイトルで正しいバージョンを表示
  * ライセンス API・X OAuth User-Agent も共通関数に統一

**新機能（v3.2.25）:**
* 2026年5月27日リリース
* 投稿設定画面の「投稿設定の追加・編集」をトグル化
  * デフォルトでフォームを折りたたみ表示
  * 編集ボタンクリック時は自動で展開
  * キャンセル時は再び折りたたみ
...
**新機能（v3.2.24）:**
* 2026年5月27日リリース
* アイキャッチ再生成時にキーワード（オプション）を優先して画像を生成
  * 投稿編集画面で入力したキーワードを AI・Unsplash・テンプレート各方式で最優先
  * DALL-E プロンプト・Unsplash 検索・テンプレート表示をキーワード中心に変更
  * キーワードを投稿メタに保存し、次回メタボックスに自動復元
  * 全角カンマ（、，）入力に対応

**新機能（v3.2.23）:**
* 2026年5月27日リリース
* アイキャッチAIモデルを設定から選択可能に
  * DALL-E 3（1792x1024 HD）と DALL-E 2（1024x1024）を切り替え可能
  * 基本設定・ジャンル設定・投稿編集メタボックス・アイキャッチ設定画面にUIを追加
  * モデルごとに API パラメータ（サイズ・品質・style）を自動切替

**新機能（v3.2.22）:**
* 2026年5月27日リリース
* 「アイキャッチを再生成」で必ず新しい画像が生成されるよう改善
  * 再生成時に旧添付ファイルと `_news_crawler_generated_image_id` メタを削除
  * AI・Unsplash・テンプレート各方式に再生成用のランダム seed を付与
  * DALL-E プロンプトに再生成専用 ID を追加して同一画像の連続生成を防止

**新機能（v3.2.21）:**
* 2026年5月27日リリース
* AIアイキャッチをプレミアムSaaSヒーロー画像品質に改善
  * 多デバイス mockup・キーノート demo・ガラスモーフィズム概念図など8種のシーンテンプレートを追加
  * 記事カード・チャート・RSS・自動化ワークフロー等の UI 要素ヒントをトピック別に付与
  * プレミアム tech SaaS ランディングページ級の品質指定をプロンプトに追加
  * 読める文字の代わりにアイコン・チャート・色ブロックで UI を表現

**新機能（v3.2.20）:**
* 2026年5月27日リリース
* AIアイキャッチをよりカラフルな画像に改善
  * 配色をネオン・レインボー・高彩度パレット中心に刷新
  * プロンプトに最大彩度・マルチカラー・目を引く色使いを明示
  * muted / monochrome / desaturated 系の旧スタイル設定を自動正規化
  * デフォルトの画像スタイルとベースプロンプトを hyper colorful 向けに更新

**新機能（v3.2.19）:**
* 2026年5月27日リリース
* AIアイキャッチをデジタルアート調に変更
  * 実写・ストックフォト風を避け、イラスト／グラフィック調の生成を明示
  * クリエイティブ指示をフラットデザイン・セルシェード・UIコンセプトアート等に刷新
  * 旧 photorealistic / editorial photography 系スタイル設定を自動正規化
  * デフォルトの画像スタイルとベースプロンプトをデジタルアート向けに更新

**新機能（v3.2.18）:**
* 2026年5月27日リリース
* AIアイキャッチのクリエイティブ生成と画像の使い回し防止を改善
  * DALL-E API に style: vivid を追加し、毎回ユニークなクリエイティブ指示（構図・比喩・配色・ライティング）を付与
  * リトライごとにプロンプトを変更し、同じ画像の連続生成を防止
  * SEOタイトルの日本語話題フレーズをプロンプトに反映して具体性を向上
  * Unsplash フォールバック時は汎用クエリを後回しにし、投稿ごとに検索順をシャッフル
  * 添付ファイル名を uniqid 化して衝突を防止

**新機能（v3.2.17）:**
* 2026年5月27日リリース
* AIアイキャッチ生成の品質と挙動を改善
  * AI選択時にテンプレートへフォールバックしないよう修正（AI → Unsplash → なし）
  * DALL-E プロンプトを明るく多様な画像向けに再設計（シーン・ムード・concept ID）
  * 旧 cinematic / dark 系スタイル設定を明るいスタイルへ自動正規化
  * 日本語キーワード辞書を拡充（金融・IT・医療・YouTube 等）

**新機能（v3.2.16）:**
* 2026年5月27日リリース
* アイキャッチ画像生成の品質を改善
  * DALL-E プロンプトを英語トピックキーワードベースに変更（長い日本語 SEO タイトルを除外）
  * AI 失敗時のフォールバックを AI → テンプレート → Unsplash の順に変更
  * Unsplash 検索は関連度の高い上位 5 件から投稿ごとに選択
  * テンプレート生成は定型レイアウト（〇〇ニュースまとめ 日付）に戻し、短いキーワードのみ描画

**新機能（v3.2.15）:**
* 2026年5月27日リリース
* OAuth 1.0a 投稿の HTTP 401 エラーを改善
  * 投稿 API を api.x.com に統一
  * Secret 復号可否を接続判定・診断パネルに反映
  * OAuth 1.0a と OAuth 2.0 の入力欄を明確化
  * 401 エラー時の対処案内を追加

**新機能（v3.2.14）:**
* 2026年5月26日リリース
* X シェアログを専用オプション（news_crawler_x_share_log）に分離
  * basic_settings 肥大化でログ保存に失敗していた問題を修正
* 未シェア投稿の再試行ボタンを追加
* シェア対象外・既シェア済みでも理由をシェアログに記録

**新機能（v3.2.13）:**
* 2026年5月26日リリース
* 投稿直後の X 自動シェアが環境差で実行されない場合のフォールバックを追加
  * `news_crawler_share_to_x` フックが未登録でも直接シェア処理を実行
  * 2記事作成されたのに X に出ないケースを回避

**新機能（v3.2.12）:**
* 2026年5月26日リリース
* 接続診断パネルが OAuth 専用オプションを参照していなかった問題を修正
  * 接続成功後も「Access Token 未保存」と誤表示されていた不具合を解消

**新機能（v3.2.11）:**
* 2026年5月26日リリース
* X OAuth 2.0 トークンを専用オプション（news_crawler_x_oauth）に分離
  * 巨大な basic_settings への保存失敗を回避
  * 接続後の Access Token 未保存問題を修正
  * 診断パネルにトークン保存先を表示

**新機能（v3.2.10）:**
* 2026年5月26日リリース
* X OAuth 2.0 接続時の HTTP 401 エラーを修正
  * Basic 認証ヘッダーを RFC 6749 準拠（rawurlencode）に変更
  * Client ID に含まれる : 文字で認証が壊れる問題を修正
  * 401 エラーメッセージを改善（API Key と OAuth 2.0 の混同を案内）
  * Client ID / Secret 入力欄に OAuth 2.0 である旨を明記

**新機能（v3.2.9）:**
* 2026年5月26日リリース
* X 接続成功後に Access Token が消える問題を修正
  * update_settings を全体上書きから既存設定とのマージ方式に変更
  * アカウント名取得失敗時に OAuth トークンが消えないよう改善
  * トークン保存後に DB 永続化を検証
  * 保存失敗時は成功通知を出さずエラーを表示

**新機能（v3.2.8）:**
* 2026年5月26日リリース
* X アカウント接続ボタンでダッシュボードに戻る問題を修正
  * X 認可画面へのリダイレクトを wp_safe_redirect から wp_redirect に変更
  * 外部 URL（x.com）への OAuth リダイレクトが正常に動作するよう修正

**新機能（v3.2.7）:**
* 2026年5月26日リリース
* X OAuth 2.0 接続フローを改善
  * 接続ボタン押下時のみ OAuth state / code_verifier を生成（ページ表示のたびに上書きされない）
  * OAuth 一時データをユーザー単位で管理
  * 接続成功時に「X アカウントに接続しました。」を表示
  * 接続診断パネルに Callback URL を表示
  * 「設定を保存だけでは接続完了しない」旨を案内

**新機能（v3.2.6）:**
* 2026年5月26日リリース
* X OAuth 2.0 の Client Secret 未設定・復号不可エラーを改善
  * トークン取得前に Client Secret の利用可否を検証
  * Missing valid authorization header を日本語の対処案内に変換
  * 接続診断パネルで Client Secret の復号状態を表示（利用可能 / 復号不可 / 未保存）
  * OAuth 2.0 トークン更新時も Client Secret 必須チェックを追加

**新機能（v3.2.5）:**
* 2026年5月26日リリース
* X 接続後のアカウント名取得失敗通知を非表示化
  * 接続成功時はアカウント名取得失敗でも通知を出さない
  * 失敗理由は debug.log に記録
  * 旧通知メッセージも表示前に抑制
* X 接続状態の診断パネルを追加
  * Client ID / Secret / Access Token / Refresh Token の保存状態を表示
  * 接続判定（接続済み / 未接続）を表示
  * 接続テスト投稿ボタンが出ない理由を案内

**新機能（v3.2.4）:**
* 2026年5月26日リリース
* X 接続エラー通知が「設定を保存」のたびに再表示される問題を修正
  * URL クエリパラメータ方式からトランジェント方式に変更
  * 一度表示した通知は自動的に消える
  * 旧バージョンの URL クエリ通知も JS で自動クリア

**新機能（v3.2.3）:**
* 2026年5月26日リリース
* GitHub リリース通知が来ない問題を修正
  * update_plugins transient が false のときの処理を追加
  * 管理画面リフレッシュ時に wp_update_plugins() を実行
  * pre_set / cron / 強制リフレッシュ時は GitHub API を直接取得
  * バックアップキャッシュの有効期限を 1 日 → 7 日に延長
  * GitHub API 403（レート制限）時にトークン設定を案内
* X 接続後にアカウント名が取得できない問題を改善
  * 取得失敗時にエラー詳細（HTTP コード）を通知に表示
  * 接続操作欄に「アカウント名を再取得」ボタンを追加
  * X API Free プラン等で取得できない場合の手動入力欄を追加
  * /2/users/me 呼び出しのデバッグログを強化
  * user.fields=username,name を明示

**新機能（v3.2.2）:**
* 2026年5月26日リリース
* X 接続後にアカウント名が表示されない問題を修正
  * 接続直後は平文トークンで /2/users/me を取得
  * api.x.com / api.twitter.com の両方を試行
  * @username と表示名（name）を保存・表示
  * 接続成功通知と接続操作欄にアカウント名を表示

**新機能（v3.2.1）:**
* 2026年5月26日リリース
* X 自動シェアが動作しない問題を修正
  * メタデータ保存後にシェア実行（投稿を作成ボタン経由でも動作）
  * 下書き→公開時は shutdown 後に実行
  * スキップ理由をシェアログに記録
* アイキャッチが毎回同じになる問題を修正
  * SEOタイトル・記事/動画タイトルをキーワードに反映
  * Unsplash 検索結果から投稿ごとに異なる画像を選択
  * テンプレート生成の配色を8種類から自動選択
  * DALL-E プロンプトに投稿固有の seed を追加

**新機能（v3.2.0）:**
* 2026年5月26日リリース
* X 接続操作に接続中アカウント名（@username）を表示
  * OAuth 2.0 / 1.0a 両対応で API からユーザー名を取得・保存
  * 接続成功時の通知に @username を含める
* X シェアログを追加
  * 投稿シェアの成功/失敗を管理画面に記録（最大50件）
  * 失敗原因（API エラー等）を専用列に表示
  * 接続テスト投稿の結果も記録
  * ログクリア機能を追加

**新機能（v3.1.9）:**
* 2026年5月26日リリース
* X（Twitter）自動シェア設定を xLabo 風 UI に刷新
  * OAuth 2.0（PKCE）認証に対応（接続・切断・テスト投稿）
  * OAuth 1.0a 認証も選択可能
  * API キー等の機密情報を AES-256-CBC で暗号化保存
  * 設定保存を NewsCrawlerSettingsManager に統一
  * X Poster クラスを OAuth 2.0 / 1.0a 両対応に再構成
  * 管理画面用 JavaScript（認証方式切替）を追加

**新機能（v3.1.8）:**
* 2026年5月26日リリース
* バージョン番号を 3.1.8 に更新（3.1.7 以降の機能変更なし）

**新機能（v3.1.7）:**
* 2026年5月26日リリース
* GitHub更新通知が表示されない問題を修正
  * プラグイン一覧・更新画面表示時にキャッシュを再取得
  * リリース一覧から semver 最大版を選ぶよう改善
  * 古いバックアップキャッシュによる更新見逃しを防止
  * 管理画面にフォールバック更新通知を追加
  * キャッシュクリア後に wp_update_plugins を実行

**新機能（v3.1.6）:**
* 2026年5月26日リリース
* Unsplash APIエラー「Content removed」の対策
  * content_filter を low に変更
  * 日本語キーワードを英語検索語へ変換
  * 検索候補を最大10件まで順次試行
  * ランダム画像APIを最終フォールバックに追加
  * AI失敗→Unsplash失敗時は投稿を継続（アイキャッチなし）

**新機能（v3.1.5）:**
* 2026年5月26日リリース
* バージョン番号を 3.1.5 に更新（3.1.4 以降の機能変更なし）

**新機能（v3.1.4）:**
* 2026年5月26日リリース
* AI（DALL-E）選択時にテンプレート画像へ自動フォールバックしないよう修正
  * AI失敗時はUnsplashへフォールバック（Unsplashも失敗時はアイキャッチなし）
  * DALL-E設定を1792x1024 / hd品質に強化
  * デフォルト生成方法をAIに統一
  * 設定値の正規化（ai_generated / template_based / external_api）
  * 生成方法を投稿メタに記録（デバッグ用）
  * 「最新最新」などタイトル重複を修正

**新機能（v3.1.3）:**
* 2026年5月26日リリース
* プラグイン更新後の再有効化と設定画面表示を修正
  * GitHub zipball 展開フォルダ（KantanPro-news-crawler-{hash}）を news-crawler に自動リネーム
  * 更新後にプラグインを自動再有効化
  * 更新完了後に News Crawler 設定画面へリダイレクト

**新機能（v3.1.2）:**
* 2026年5月26日リリース
* GitHubリリース連携の更新通知取得を改善
  * WordPress標準の更新通知＋zipball_url対応（assetなしリリースでも更新可能）
  * 更新後の自動再有効化・安全リロード・フォルダ名正規化を整理
  * シングルトン化で二重フック登録を防止
  * 過剰なキャッシュクリアを削除し更新通知の安定性を向上
  * KP_GITHUB_TOKEN / NEWS_CRAWLER_GITHUB_TOKEN 対応

**新機能（v3.1.1）:**
* 2026年5月26日リリース
* アイキャッチ画像生成の不具合を修正
  * 同梱日本語フォント（Noto Sans JP）を正しいフォントファイルに差し替え（破損HTMLファイルを除去）
  * グラデーション背景・テキスト描画処理の戻り値不備を修正
  * AI/Unsplash失敗時にテンプレート生成へフォールバック
  * プラグイン同梱フォントを優先し、Linuxサーバーでも日本語描画可能に改善
  * 生成方法の正規化とテンプレート生成オプションを追加

**新機能（v3.1.0）:**
* 2025年9月30日リリース
* 投稿フックの登録強化とX投稿の自動実行ロジック改善
  * 投稿フックの登録を強化し、各フックの登録状況をログに記録する機能を追加
  * 投稿挿入後や更新時の処理を追加し、公開状態の投稿に対してX投稿を自動実行するロジックを改善
  * 投稿公開時のX投稿処理の信頼性と安定性を大幅に向上
  * フック登録状況の詳細なログ記録により、デバッグとトラブルシューティングを効率化
  * 投稿ライフサイクル全体でのX投稿機能の一貫した動作を保証
  * プラグインのソーシャルメディア連携機能の堅牢性を強化

**新機能（v3.0.9）:**
* 2025年9月30日リリース
* 投稿作成数記録機能の初期化処理の改善
  * 自動投稿機能において、投稿作成数を記録するための変数を初期化する処理を追加
  * 投稿数カウントの正確性を向上させ、ログ記録の信頼性を改善
  * 自動投稿実行時のデータ整合性を確保し、より安定した動作を実現
  * プラグインの統計機能とモニタリング機能の精度を向上
  * デバッグとトラブルシューティングの効率化を実現

**新機能（v3.0.8）:**
* 2025年9月30日リリース
* 自動投稿機能のログ記録機能の強化
  * 自動投稿機能において、作成された投稿数を記録する変数を追加し、最終結果をログに記録する処理を強化
  * 投稿作成プロセスの詳細な追跡とモニタリング機能を改善
  * 自動投稿の実行結果をより詳細に記録し、問題の特定と解決を容易に
  * プラグインの動作状況をより正確に把握できるように改善
  * デバッグとトラブルシューティングの効率化を実現

**新機能（v3.0.7）:**
* 2025年9月29日リリース
* 設定画面の改善とユーザビリティの向上
  * 自動投稿設定に関する説明文を削除し、設定画面をより簡潔で分かりやすく改善
  * ユーザーインターフェースの整理により、設定操作をより直感的に
  * 不要な説明文を削除することで、重要な設定項目に集中できる環境を提供
  * プラグインの設定管理をより効率的で使いやすく改善

**新機能（v3.0.6）:**
* 2025年9月29日リリース
* エラーログ機能の改善とデバッグ機能の強化
  * 投稿ステータス変更時のフックを追加し、公開時にX投稿を実行する際のエラーログを追加
  * cronエラーログに同じエラーメッセージが繰り返し記録される問題を修正
  * プラグインのデバッグ機能を強化し、問題の特定と解決を容易に
  * エラーログの重複を防ぎ、ログファイルの可読性と管理性を向上
  * 開発者とユーザーの両方にとってより使いやすいデバッグ環境を提供
