...
- **ニュースクロール機能の設定改善**: 最大記事数の代わりに一度に引用する記事数を指定するオプションを追加
- **管理画面のコンテンツタイプ設定改善**: デフォルト値を適切に設定し、より使いやすい設定画面を実現
- **SEOタイトル自動生成機能**: 管理画面に設定オプションを実装し、SEO最適化を自動化
- **要約挿入位置の改善**: 要約を最初のH2タグの上に挿入し、まとめを投稿内容の末尾に追加
